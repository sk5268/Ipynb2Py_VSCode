...
export function activate(context: vscode.ExtensionContext) {
  ...
  const exportToPython = vscode.commands.registerCommand('ipynb2py.exportToPython', async () => {
    ...
    try {
      ...
      // Check if file exists and whether confirmation is needed
      const fileExists = fs.existsSync(pythonPath);
      const config = vscode.workspace.getConfiguration('ipynb2py');
      const confirmOverwrite = config.get<boolean>('confirmOverwrite', true);
      
      // Determine whether to proceed with export
      ...
      if (fileExists && confirmOverwrite) {
        ...
        shouldProceed = (overwriteChoice === 'Yes');
      }
